...
class PSFWrapper(object):
    ...
    def find_eef_radius(self, fraction):
        ...
        # 定义一个绝对的最大搜索半径，例如5度。
        # 即使积分到这里还没达到fraction，我们也不再往外找了。
        MAX_SEARCH_RADIUS = min(5.0, _INTEGRAL_OUTER_RADIUS)

        try:
            radius, status = scipy.optimize.brentq(
                f, 0.005, MAX_SEARCH_RADIUS, full_output=True
            )
            assert status.converged, "Brentq did not converged"
        except ValueError:
            # 当 f(a) 和 f(b) 符号相同时，brentq会抛出ValueError。
            # 这通常意味着积分到 MAX_SEARCH_RADIUS 也没能达到要求的 fraction。
            # 在这种情况下，我们直接返回搜索上限作为半径。
            # print(f"Warning: Could not find radius for EEF={fraction} within {MAX_SEARCH_RADIUS} deg. Capping at max search radius.")
