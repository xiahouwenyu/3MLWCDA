...
# @guvectorize([(float64[:,:], float64[:], float64[:])], '(n,m),(m)->(n)', nopython=True, target='parallel')
# def custom_einsum(mat, vec, out):
...
#     ss1_sum = custom_einsum(sflux, ss1, ss1_sum)
#     ss2_sum = custom_einsum(sflux, ss2, ss2_sum)
#     return (ss1_sum * w1 + ss2_sum * w2) * 1e9

# 方法们：Numba 加速
@njit(parallel=True, fastmath=True)
def method_numba(A, B):
    N = A.shape[0]
...
def method_vectorized(A, B):
    return np.sum(multiply_and_sum(A, B).reshape(A.shape), axis=1)
...
# @jit(["float64[:](float64[:,:], float64[:], float64[:], float64[:], float64[:])"], parallel=False,nopython=False)
# def process_dec_bins(sflux, w1, w2, ss1, ss2):
...
# @njit(parallel=True)
# def process_dec_bins(sflux, w1, w2, ss1, ss2):
#     ss1_sum = np.zeros(sflux.shape[0])
#     ss2_sum = np.zeros(sflux.shape[0])
    
#     # 并行化计算 ss1_sum
#     for i in prange(sflux.shape[0]):
#         total = 0.0
#         for j in range(sflux.shape[1]):
#             total += sflux[i,j] * ss1[j]
#         ss1_sum[i] = total * w1[i]
    
#     # 并行化计算 ss2_sum
#     for i in prange(sflux.shape[0]):
#         total = 0.0
#         for j in range(sflux.shape[1]):
#             total += sflux[i,j] * ss2[j]
#         ss2_sum[i] = total * w2[i]
    
#     return ss1_sum + ss2_sum
...
class ConvolvedExtendedSource(object):
    ...
    def __init__(self, source, response, flat_sky_projection):
        ...
        #     # This can happen if for instance lat_start = 280 and lat_stop = 10, which means we
        #     # should keep between 280 and 360 and then between 0 to 10
        #     lat_start, lat_stop = lat_stop, lat_start
        
        decs = np.array(self._flat_sky_projection.decs)
        ...
        self.idx_lon = _select_with_wrap_around(self._flat_sky_projection.ras, lon_start, lon_stop, (360, 0))
        self.idx_lat = _select_with_wrap_around(self._flat_sky_projection.decs, lat_start, lat_stop, (90, -90))

        self._active_flat_sky_mask = (self.idx_lon & self.idx_lat)
        ...
        self._sim_differential_photon_fluxes = self._central_response_bins[list(self._central_response_bins.keys())[0]].sim_differential_photon_fluxes

        # self._lenra = len(self.idx_lon[self.idx_lon==1])
        # self._lenlat = len(self.idx_lat[self.idx_lat==1])

        self._active_pixels = len(self._active_flat_sky_mask[self._active_flat_sky_mask>0])

        self._pixel_area = self._flat_sky_projection.project_plane_pixel_area

        # Prepare array for fluxes
        self._all_fluxes = np.zeros((self._active_pixels,
                                     self._energy_centers_keV.shape[0]), order='C')
        
        self.model_images = np.zeros((self._response.n_energy_planes, self._active_pixels), order='C')
...
class ConvolvedExtendedSource3D(ConvolvedExtendedSource):
    ...
    def __init__(self, source, response, flat_sky_projection):
        ...
        self._recompute_flux = np.ones(self._response.n_energy_planes, dtype=bool)
        ...
        def get_responses(energy_bin_id, dec_bin1, dec_bin2):
            ...
            idx = (self._flat_sky_projection.decs[self._active_flat_sky_mask] >= c1) & (self._flat_sky_projection.decs[self._active_flat_sky_mask] < c2)

            ss1 = this_response_bin1.sim_signal_events_per_bin* self._pixel_area * deg2_to_rad2 / self._sim_differential_photon_fluxes
            ss2 = this_response_bin2.sim_signal_events_per_bin* self._pixel_area * deg2_to_rad2 / self._sim_differential_photon_fluxes

            w1 = (self._flat_sky_projection.decs[self._active_flat_sky_mask][idx] - c2) / (c1 - c2)
            w2 = (self._flat_sky_projection.decs[self._active_flat_sky_mask][idx] - c1) / (c2 - c1)

            # A = np.stack([w1, w2], axis=0)
            # B = np.stack([ss1, ss2], axis=1)
            # C = np.einsum('ik,kj->ij', B, A)

            A = np.stack([w1, w2], axis=1)   # shape: (N, 2)
            B = np.stack([ss1, ss2], axis=0) # shape: (2, 70)
            C = np.einsum('ik,kj->ij', A, B) * 1e9 # result: (N, 70)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            for energy_bin_id in range(self._response.n_energy_planes):
                ...
                result = np.zeros((self._active_pixels,
                            self._energy_centers_keV.shape[0]), order='C')
                ...
                self._responsemap.append(result)
    ...
    def get_source_map(self, energy_bin_id, tag=None):
        ...
        with use_astromodels_memoization(False):
            ...
            if self._recompute_flux[int(energy_bin_id)]:
            ...
            #         return self._this_model_image[int(energy_bin_id)]
                ...
                # Recompute the fluxes for the pixels that are covered by this extended source
                self._all_fluxes = self._source(
                                self._flat_sky_projection.ras[self._active_flat_sky_mask],
                                self._flat_sky_projection.decs[self._active_flat_sky_mask],
                                self._energy_centers_keV)  # 1 / (keV cm^2 s rad^2)
                
                # We don't need to recompute the function anymore until a parameter changes
                self._recompute_flux[int(energy_bin_id)] = False
                recomputed = True
                
                # for method in [method_sum, method_einsum, method_numba_opt, method_numba, method_guvectorize, method_vectorized]:
                #     start = time.time()
                #     this_model_image = method(np.ascontiguousarray(self._all_fluxes), np.ascontiguousarray(self._responsemap[int(energy_bin_id)]))
                #     print(f"{method.__name__}: {(time.time() - start):.5f}s")

                this_model_image = method_guvectorize(np.ascontiguousarray(self._all_fluxes), np.ascontiguousarray(self._responsemap[int(energy_bin_id)]))
                self.model_images[int(energy_bin_id)] = this_model_image
                # print(f"END {time.time()}: {(time.time() - start):.5f}s")
                return self._active_flat_sky_mask, self.model_images[int(energy_bin_id)]
            else:
                # print("reusing %s" % self._name)

                # If we do not need to recompute the flux, we can just return the cached value
                # NOTE: this is a bit of a hack, but it works because we are using the same energy bin id for the
                # response and for the source map
                return self._active_flat_sky_mask, self.model_images[int(energy_bin_id)]
            ...
            # self._flat_sky_projection.npix_height, self._flat_sky_projection.npix_width
            ...
            #     results = list(executor.map(lambda bins: process_dec_bins(self._all_fluxes[bins[0]], *bins[1:]), self._resultss[int(energy_bin_id)]))
            ...
            # print(np.shape(this_model_image))
            # print(np.shape(this_model_image.reshape((self._flat_sky_projection.npix_height,
            #                                              self._flat_sky_projection.npix_width)).T))
            # from IPython import embed; embed()
            # self._this_model_image[int(energy_bin_id)] =  this_model_image.reshape((self._flat_sky_projection.npix_height,
            #                                              self._flat_sky_projection.npix_width)).T
