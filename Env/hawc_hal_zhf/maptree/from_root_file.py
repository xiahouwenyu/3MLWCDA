...
class MaptreeMetaData:
    ...
    #         print("type of analysis_bin_names[0]: ",type(self.maptree_ttree_directory["BinInfo/name"].array().to_numpy()[0]))
    ...
    def analysis_bin_names(self) -> NDArray[np.string_]:
        """Get the analysis bin names contained within the maptree"""
        if self.maptree_ttree_directory.get("BinInfo/name", None) is not None:
            bin_names = self.maptree_ttree_directory["BinInfo/name"].array().to_numpy()
            # print("type of analysis_bin_names[0]: ",type(self.maptree_ttree_directory["BinInfo/name"].array().to_numpy()[0]))
            return bin_names.astype(np.str_)

        if self.maptree_ttree_directory.get("BinInfo/id", None) is not None:
            bin_ids = self.maptree_ttree_directory["BinInfo/id"].array().to_numpy()
            return bin_ids.astype(np.str_)
