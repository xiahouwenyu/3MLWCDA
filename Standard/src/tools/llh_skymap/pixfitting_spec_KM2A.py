...
if __name__ == "__main__":
    ...
    p.add_argument("--s", dest="s", default=2, type=int,help="Starting analysis bin [0..13]")
    p.add_argument("--e", dest="e", default=7, type=int,help="Starting analysis bin [0..13]")
    ...
    name=args.name
    if not os.path.exists(f'{outdir}/sourcetxt/KM2A_{name}/'):
        os.system(f'mkdir {outdir}/sourcetxt/KM2A_{name}/')
    np.save(f"{outdir}/sourcetxt/KM2A_{name}/{name}_{ii}.npy", results)