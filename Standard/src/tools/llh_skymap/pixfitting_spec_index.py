...
#    pixid=roi.active_pixels(roi._original_nside)
  ...
  # for i in range(len(pixid)):
    ...
    spectrum.K.bounds=(-1e-13*fluxUnit, 1e-11*fluxUnit)
    spectrum.piv= 3.*u.TeV
    spectrum.piv.fix=True
    spectrum.index=-2.6
    spectrum.index.bounds=(-6,1)
...
#    for pid in range(args.StartPix,args.StopPix):    
        ...
        try:
            ...
            index = results.optimized_model.Pixel.spectrum.main.Powerlaw.index.value
            sig = K_fitted
            # if(ts>=0):
            #     if(K_fitted>=0):
            #         sig=np.sqrt(ts)
          ...
          #  sig_list.append(sig)
        with open("/data/home/cwy/Science/3MLWCDA/Standard/src/tools/llh_skymap/skytxt/sig_no%i.txt"%no,"a+") as fs:
            fs.write(str(pid)+" "+str(sig)+" "+str(index)+"\n")
